...
contract ValidatorSetHbbft is UpgradeableOwned, IValidatorSetHbbft {
    ...
    function finalizeChange() external onlyBlockRewardContract {
        ...
        // new epoch starts
        IStakingHbbft(stakingContract).incrementStakingEpoch();
        keyGenHistoryContract.notifyNewEpoch();
        delete _pendingValidators;
        IStakingHbbft(stakingContract).setStakingEpochStartTime(
        ...
        external
    {
        ...
        require(
            IStakingHbbft(stakingContract).isPoolActive(
        ...
        if (!isValidatorBanned(msg.sender)) {
            IStakingHbbft(stakingContract).notifyAvailability(
                stakingByMiningAddress[msg.sender]
            );
        }
    }
    ...
    function handleFailedKeyGeneration() external onlyBlockRewardContract {
        ...
        require(
            this.getCurrentTimestamp() >=
                IStakingHbbft(stakingContract).stakingFixedEpochEndTime(),
            "failed key generation can only be processed after the staking epoch is over."
        );

        if (IStakingHbbft(stakingContract).getPoolsToBeElected().length == 0) {
        ...
        for (uint256 i = 0; i < _pendingValidators.length; i++) {
            ...
            if (isGood) {
                ...
                IStakingHbbft(stakingContract).removePool(
        ...
        IStakingHbbft(stakingContract).notifyKeyGenFailed();
    ...
    function setValidatorInternetAddress(bytes16 _ip, bytes2 _port) external {
        ...
        IStakingHbbft(stakingContract).setValidatorInternetAddress(
            validatorAddress,
            _ip,
            _port
        );
        ...
        returns (bool)
    {
        ...
        if (IStakingHbbft(stakingContract).stakingEpoch() == 0) {
            return isValid;
        }
        // TO DO: arbitrarily chosen period stakingFixedEpochDuration/5.
        if (
            this.getCurrentTimestamp() -
                IStakingHbbft(stakingContract).stakingEpochStartTime() <=
            IStakingHbbft(stakingContract).stakingFixedEpochDuration() / 5
        ) {
    ...
    /// @return callable `bool callable` - The boolean flag indicating whether the `reportMalicious` function can be
    /// called at the moment. `bool removeReportingValidator` - The boolean flag indicating whether the
    /// reporting validator should be removed as malicious due to excessive reporting. This flag is only used
    ...
    function reportMaliciousCallable(
        ...
        if (validatorsNumber > 1) {
            uint256 currentStakingEpoch = IStakingHbbft(stakingContract)
                .stakingEpoch();
    ...
    {
        return IStakingHbbft(stakingContract).getPoolPublicKey(_stakingAddress);
    }
    ...
    {
        return
            IStakingHbbft(stakingContract).getPoolPublicKey(
    ...
    function _newValidatorSet(address[] memory _forcedPools) internal {
        address[] memory poolsToBeElected = IStakingHbbft(stakingContract)
            .getPoolsToBeElected();
        ...
        if (poolsToBeElected.length > numOfValidatorsToBeElected) {
            ...
            ) = IStakingHbbft(stakingContract).getPoolsLikelihood();
            ...
            for (
                ...
                for (
                    ...
                    if (
                        ...
                        delete poolsToBeElected[poolsToBeElectedLength];
        ...
        if (poolsToBeElected.length != 0) {
            ...
            IStakingHbbft(stakingContract).removePools();
        }
        ...
        // current end of phase 2 : IStakingHbbft(stakingContract).stakingFixedEpochEndTime()
        ...
        if (
            ...
            IStakingHbbft(stakingContract).notifyNetworkOfftimeDetected(
                this.getCurrentTimestamp() -
                    IStakingHbbft(stakingContract).stakingFixedEpochEndTime()
            );
        }
        ...
        returns (bool)
    {
        ...
        IStakingHbbft(stakingContract).removePool(stakingAddress);
        ...
        if (_stakingAddresses.length == 0) {
            ...
            for (uint256 i = 0; i < _currentValidators.length; i++) {
                ...
                if (
                    ...
                    IStakingHbbft(stakingContract).orderedWithdrawAmount(
                        pvStakingAddress,
                        pvStakingAddress
                    ) ==
                    0
    ...
    function _banUntil() internal view returns (uint256) {
        ...
        uint256 ticksUntilEnd = IStakingHbbft(stakingContract)
            .stakingFixedEpochEndTime() - (currentTimestamp);
        // Ban for at least 12 full staking epochs:
        // currentTimestampt + stakingFixedEpochDuration + remainingEpochDuration.
        return
            currentTimestamp +
            (12 * IStakingHbbft(stakingContract).stakingFixedEpochDuration()) +
            (ticksUntilEnd);
    }
